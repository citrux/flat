...
struct Data {
  ...
  Data(int waves, int bands)
      : v(Vec2(0, 0)), power(0), tau(0), acoustic_phonon_scattering_count(0),
        optical_phonon_scattering_count(0), vertical_transitions_count(0) {
    power = std::vector<double>(waves);
    population = std::vector<double>(bands);
  };
};
...
bool operator>(std::vector<double> &lhs, double rhs) {
  int len = lhs.size();
  for (int i = 0; i < len; i++) {
    if (lhs[i] <= rhs)
      return false;
...
std::vector<double> sqrt(std::vector<double> lhs) {
  for (auto &x : lhs) {
    x = sqrt(x);
  }
  return lhs;
...
template <typename T> inline Data &operator*=(Data &lhs, T rhs) {
  lhs.v *= rhs;
  lhs.tau *= rhs;
  lhs.power *= rhs;
...
template <typename T> inline T mean(std::vector<T> const &values) {
  T res = values[0];
  res -= values[0];
  for (T x : values) {
    res += x;
...
template <typename T> inline T disp(std::vector<T> const &values) {
  T m = mean(values);
...
template <typename T> inline T stdev(std::vector<T> const &values) {
