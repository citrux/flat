...
Bigraphene::Bigraphene(double temperature, double delta, double number_of_bands) : delta(delta) {
...
double Bigraphene::vertical_transition(Particle * p, Band *dest, Wave const & wave, double de) {
    ...
    double b = (std::pow(e1 + d, 2) - p2) * (std::pow(e2 + d, 2) - p2) / wave.omega;
    ...
    double wtf = p2 * std::pow((p2 - 4 * d * d) / (e1 + d) / (e2 * e2 - d * d), 2);
    double norm1 = g2 * std::pow(e1 - d, 2) + g2 * std::pow(e1 + d, 2) * wtf + (wtf + 1) * std::pow(std::pow(e1 + d, 2) - p2, 2);
    double norm2 = g2 * p2 * std::pow(e2 - d, 2) + g2 * std::pow(e2 * e2 - d * d, 2) + (std::pow(e2-d,2) + p2) * std::pow(std::pow(e2 + d, 2) - p2, 2);
...
Lower::Lower(double temperature, double _delta) : delta(_delta) {
    ...
    const double crit_momentum = delta * std::sqrt(2 - 4 * delta * delta / (gamma * gamma + 4 * delta * delta));
    ...
    for (int i = 1; i < energy_samples; ++i) {
        double e = (min_energy() * (energy_samples - 1 - i) + max_energy * i) / (energy_samples - 1);
        table[i].energy = e;
        if (e < delta) {
            double p = bisection([this,e](double p){return energy(p) - e;}, 0, crit_momentum, momentum_precision);
            table[i].integrals.push_back({p,
                    2.0f * pi * p / (double) std::sqrt(std::pow(velocity(p), 2) + 1e-3f)});
        }
        if (e < max_energy) {
            double p = bisection([this,e](double p){return energy(p) - e;}, crit_momentum, max_momentum, momentum_precision);
            table[i].integrals.push_back({p,
                    2.0f * pi * p / (double)std::sqrt(std::pow(velocity(p), 2) + 1e-3f)});
        }
    }
...
Upper::Upper(double temperature, double _delta) : delta(_delta) {
    ...
    for (int i = 0; i < energy_samples; ++i) {
        double e = (min_energy() * (energy_samples - 1 - i) + max_energy * i) / (energy_samples - 1);
        table[i].energy = e;
        double p = bisection([this,e](double p){return energy(p) - e;}, 0, max_momentum, momentum_precision);
